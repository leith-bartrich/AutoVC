...
def main(
    ...
    # we load the previously trained model rather than create a new one.
    model = torch.jit.load(save_dir / f"model-{resume_step}.pt").to(device)
    # set to training mode.  may not be necessary.
    model.train(True)
    ...
    optimizer_state_dict = torch.load(save_dir / f"optimizer-{resume_step}.pt")
    ...
    # we start from the next step
    pbar = trange(resume_step + 1, n_steps, 1)
    for step in pbar:
        try:
